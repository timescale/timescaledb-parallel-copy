...
var recoveryTimeout = 5 * time.Minute
...
func (c *Copier) processBatches(ctx context.Context, ch chan Batch, workerID int) (err error) {
	dbx, err := handleRecoverableErrors[*sqlx.DB](c, func() (*sqlx.DB, error) { return connect(c.connString) }, 0)
	if err != nil {
		return err
	}
	defer dbx.Close()
...
func (c *Copier) handleBatch(ctx context.Context, batch Batch, dbx *sqlx.DB, copyCmd string, depth int) error {
	...
	var err error
	start := time.Now()
	rows, err = handleRecoverableErrors(c, func() (int64, error) {
...
func handleRecoverableErrors[T any](c *Copier, f func() (T, error), depth int) (T, error) {
	result, err := f()
	if err != nil && isRecoverableError(err) && c.retryOnRecoverableError {
		...
		} else if time.Since(*recoveryStartedAt) > recoveryTimeout {
			c.Logger.Infof("Timing out after %.0f seconds", recoveryTimeout.Seconds())
			return result, err
		}
		sleepDuration := (1 << depth) * time.Second
		...
		result, err = handleRecoverableErrors[T](c, f, min(depth+1, 5))
		if err == nil {
