...
func NewCopier(
	...
	copier := &Copier{
		...
		useDirectCompress:    true,
		hasClientSideSorting: false,
	}
...
func (c *Copier) processBatches(ctx context.Context, ch chan Batch, workerID int) (err error) {
	...
	if c.verbose {
		...
		c.LogInfo(ctx, fmt.Sprintf("setting direct compress to '%t' for the session", c.useDirectCompress))
		c.LogInfo(ctx, fmt.Sprintf("setting client side sorting to '%t' for the session", c.hasClientSideSorting))
	}

	// set Direct Compress GUCs for session
	if _, err := dbx.Exec(fmt.Sprintf("SET timescaledb.enable_direct_compress_copy=%t", c.useDirectCompress)); err != nil {
		return err
	}
	// set Direct Compress's client side sorting GUCs for session
	if _, err := dbx.Exec(fmt.Sprintf("SET timescaledb.enable_direct_compress_copy_client_sorted=%t", c.hasClientSideSorting)); err != nil {
		return err
	}
